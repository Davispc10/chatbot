...
const verificarUsuario = (ctx, next) => {
  const mesmoIDMsg = ctx.update.message &&
    ctx.update.message.from.id === env.id
  const mesmoIDCallback = ctx.update.callback_query &&
    ctx.update.callback_query.from.id === env.id

  if (mesmoIDMsg || mesmoIDCallback) {
    next()
  } else {
    ctx.reply('Desculpe, não fui autorizado a conversar com voce!')
  }
}

// ter mais de um middleware
